...
public final class BeanUtils implements Serializable {
	 ...
	 * FIXME Not initializing non-primitive variable or initializing variable by any
	 * another method.<br>
	 * Because while obfuscating code, it'll generate _clinit@12345677() method for
	 * referencing.<br>
	 * This method will be failed on android (not support) or referencing from
	 * another class.<br>
	 * So, solution is creating a static method for initializing this variable and
	 * synchronizing it.
	 */
		...
		if (primitiveTypes == null) {
			primitiveTypes = new Class<?>[][] {
					...
					{ java.lang.Byte.class, java.lang.Short.class, java.lang.Integer.class, java.lang.Long.class,
							java.lang.Float.class, java.lang.Double.class, java.lang.Boolean.class,
	...
	private static BeanUtilsBean getBeanUtils() {
		if (beanUtilsBean == null) {
			beanUtilsBean = new BeanUtilsBean(new ConvertUtilsBean(), BeanUtilsBean.getInstance().getPropertyUtils());
		}
	...
	protected BeanUtils() {
		...
		getBeanUtils().getConvertUtils().register(new DateConverter(null), java.util.Date.class);
		getBeanUtils().getConvertUtils().register(new SqlDateConverter(null), java.sql.Date.class);
		getBeanUtils().getConvertUtils().register(new SqlTimeConverter(null), java.sql.Timestamp.class);
		getBeanUtils().getConvertUtils().register(new BigDecimalConverter(null), BigDecimal.class);
	}
	 ...
	 * Gets java.lang type of the specified primitive type
	 *
	 * @param primitiveClassName the primitive type class name
	 *
	 * @return the java.lang type
	 */
	 ...
	 * @param srcList         source beans list
	 * @param clazz           source bean class
	 * @param ignorePoperties the ignored property names that has not been cloned
	 *                        value
	 *
	 * @return destination beans list
	 * @throws Exception thrown if fail
	 */
	public static <T, K> List<T> copyBeansList(List<K> srcList, Class<T> clazz, String... ignorePoperties)
			throws Exception {
	 ...
	 * @throws Exception thrown if fail
	 */
	public static <T, K> List<T> copyBeansListByFields(List<K> srcList, Class<T> clazz, String... ignoreFields)
			throws Exception {
		List<T> destDtos = new LinkedList<T>();
		if (!CollectionUtils.isEmpty(srcList)) {
			for (K src : srcList) {
	...
	public static <T, K> void copyFields(K src, T dest, String... ignoreFields) throws Exception {
		...
		try {
			...
			for (Field srcFd : srcFds) {
				if (ignoreFields == null || !ignoreList.contains(srcFd.getName())) {
					int srcModifiers = srcFd.getModifiers();
					if (Modifier.isFinal(srcModifiers))
						continue;
					boolean srcAccessible = srcFd.isAccessible();
					if (!Modifier.isPublic(srcModifiers) && !Modifier.isStatic(srcModifiers))
					...
					if (!Modifier.isPublic(destModifiers) && !Modifier.isStatic(destModifiers))
						destFd.setAccessible(true);
	...
	public static <T> boolean isInstanceOf(Object value, Class<? extends T> checkedCls) {
		...
		return ((checkedCls != null && checkedCls.isInstance(value)) || isInstanceOf(valueClass, checkedCls));
	}
	...
	public static <T> boolean isInstanceOf(Object value, Class<? extends T>... checkedClss) {
		if (CollectionUtils.isEmpty(checkedClss))
			return false;
		for (Class<? extends T> checkedCls : checkedClss) {
			if (isInstanceOf(value, checkedCls))
				return true;
		}
		return false;
	}
	...
	public static <T> boolean isInstanceOf(Class<?> valueClass, Class<? extends T>... checkedClss) {
		if (valueClass == null || CollectionUtils.isEmpty(checkedClss))
			return false;
		for (Class<? extends T> checkedCls : checkedClss) {
			if (isInstanceOf(valueClass, checkedCls))
				return true;
		}
		return false;
	}
	...
	/**
	 * Gets a class in the specified classes array that is super or instance of the
	 * specified value
	 *
	 * @param <T>         the instance type
	 * @param value       the value to check
	 * @param checkedClss the class to check
	 *
	 * @return the class in the specified classes array that is super or instance of
	 *         the specified value. NULL for not found
	 */
	...
	public static <T> boolean isInstanceOf(Class<?> valueClass, Class<? extends T> checkedCls) {
		...
		if ((checkedCls != null && valueClass != null) && (checkedCls.isAssignableFrom(valueClass)
				|| checkedCls == valueClass || checkedCls.equals(valueClass))) {
			return true;
			...
			if (CollectionUtils.indexOf(valueClass.getInterfaces(), checkedCls) != -1) {
				...
				// check whether one of interfaces of value class extends from checked class
				// interface
			} else {
...
//				LogUtils.logError(BeanUtils.class, e.getMessage());
	...
	public static void copyProperties(Object dest, Object orig) {
		if (dest != null && orig != null) {
			try {
				getBeanUtils().copyProperties(dest, orig);
			} catch (Exception e) {
//				LogUtils.logError(BeanUtils.class, e.getMessage());
			}
		}
	 ...
	 * @param dest             destination bean
	 * @param orig             source bean
	 * @param ignoreProperties the ignored property names that has not been cloned
	 *                         value
	 */
...
//				LogUtils.logError(BeanUtils.class, e.getMessage());
	 ...
	 */
	public static <T extends Annotation> T getClassAnnotation(Class<?> targetClass,
	 ...
	 * @return the annotation of the specified entity class and annotation class
	 */
	public static <T extends Annotation> T getClassAnnotation(String targetClassName,
			Class<? extends T> annotationClass) {
	...
	public static Object newInstance(Class<?> beanClass, Object... args) {
		...
		if (beanClass != null) {
			...
			for (int i = 0; i < primitiveTypes[1].length; i++) {
				if (primitiveTypes[1][i].equals(beanClass) || isInstanceOf(beanClass, primitiveTypes[1][i])) {
...
//					LogUtils.logError(BeanUtils.class, e.getMessage());
				...
				if (!CollectionUtils.isEmpty(methods)) {
					for (Method tmpMed : methods) {
						...
						if (name.equalsIgnoreCase(tmpMed.getName())) {
							boolean valid = (CollectionUtils.getSize(paramTypes) == CollectionUtils
									.getSize(parameterTypes));
							if (valid && !CollectionUtils.isEmpty(paramTypes)) {
								...
								while (medParamTypes.size() > 0 && argParamTypes.size() > 0) {
								...
								valid = (CollectionUtils.getSize(paramTypes) == CollectionUtils
										.getSize(parameterTypes));
								if (valid) {
									method = tmpMed;
									break;
	...
	/**
	 * Get the method by the specified entity class and method name (ignore
	 * case-sensitive)
	 *
	 * @param beanClassName  entity class name to check
	 * @param name           method name
	...
	/**
	 * Get the method by the specified entity instance and method name (ignore
	 * case-sensitive)
	 *
	 * @param bean           entity instance to check
	 * @param name           method name
	...
	/**
	 * Gets the annotations of the methods in the specified entity class and
	 * annotation class
	 *
	 * @param targetClassName   the instance class name to check
	 ...
	 * @param parameterTypes    the method parameter types
	 * @param ignoreCase        specify whether ignoring case-sensitive when
	 *                          comparing
	 *
	 * @return the annotations list of the specified method
	 */
	public static List<? extends Annotation> getMethodAnnotations(String targetClassName, String methodName,
			boolean ignoreCase, Class<?>[] parameterTypes, Class<?>... annotationClasses) {
	 ...
	 * @return the annotations list of the specified method
	 */
	public static List<? extends Annotation> getMethodAnnotations(Class<?> targetClass, String methodName,
			boolean ignoreCase, Class<?>[] parameterTypes, Class<?>... annotationClasses) {
	 ...
	 */
	public static List<? extends Annotation> getMethodAnnotations(String targetClassName, String methodName,
			boolean ignoreCase, Class<?>... annotationClasses) {
		return getMethodAnnotations(safeClass(targetClassName), methodName, ignoreCase, annotationClasses);
	}
	...
	public static List<? extends Annotation> getMethodAnnotations(Class<?> targetClass, String methodName,
			boolean ignoreCase, Class<?>... annotationClasses) {
		return getMethodAnnotations(targetClass, methodName, ignoreCase, (Class<?>[]) null, annotationClasses);
	}

	/**
	 * Gets the annotations of the methods in the specified entity class and
	 * annotation class (ignore case-sensitive)
	 *
	 * @param targetClassName   the instance class name to check
	 * @param methodName        the method name to parse.
	 * @param annotationClasses the annotation class to check
	...
	public static List<? extends Annotation> getMethodAnnotations(Class<?> targetClass, String methodName,
			Class<?>[] parameterTypes, Class<?>... annotationClasses) {
		return getMethodAnnotations(targetClass, methodName, Boolean.TRUE, parameterTypes, annotationClasses);
	}
	...
	public static List<? extends Annotation> getMethodAnnotations(Class<?> targetClass, String methodName,
			Class<?>... annotationClasses) {
		return getMethodAnnotations(targetClass, methodName, Boolean.TRUE, (Class<?>[]) null, annotationClasses);
	}

	/**
	 * Gets the annotation of the methods in the specified entity class and
	 * annotation class
	 *
	 * @param <T>             annotation type
	 * @param targetClassName the instance class name to check
	 ...
	 * @return the annotations list of the specified method
	 */
	public static <T extends Annotation> T getMethodAnnotation(String targetClassName, String methodName,
			boolean ignoreCase, Class<?>[] parameterTypes, Class<T> annotationClass) {
	 ...
	 */
	public static <T extends Annotation> T getMethodAnnotation(String targetClassName, String methodName,
			boolean ignoreCase, Class<T> annotationClass) {
...
//				LogUtils.logError(BeanUtils.class, e.getMessage());
	 ...
	 * @return the returned method value if necessary
	 */
	public static Object invokeMethod(Object target, String name, boolean ignoreClass, Class<?>[] parameterTypes,
			Object[] args) {
	...
	/**
	 * Get the field by the specified entity class and field name (ignore
	 * case-sensitive)
	 *
	 * @param beanClassName entity class name to parse
	 * @param name          declared field name to parse
	...
	/**
	 * Get the field by the specified entity instance and field name (ignore
	 * case-sensitive)
	 *
	 * @param bean entity instance to check
	 * @param name declared field name to parse
	...
	/**
	 * Gets the annotation of the field in the specified entity class, field name
	 ...
	 */
	public static <T extends Annotation> T getFieldAnnotation(String targetClassName, String fieldName,
			Class<T> annotationClass, boolean ignoreCase) {
	 ...
	 * @return the annotations list of the specified field
	 */
	public static <T extends Annotation> T getFieldAnnotation(Class<?> targetClass, String fieldName,
			Class<T> annotationClass, boolean ignoreCase) {
	...
	/**
	 * Gets the annotation of the field in the specified entity class, field name
	 * and annotation class (ignore case-sensitive)
	 *
	 * @param <T>             annotation type
	 * @param targetClassName the instance class name to check
	 * @param fieldName       the field name to parse.
	...
	/**
	 * Gets the annotation of the field in the specified entity, field name and
	 * annotation class
	 *
	 * @param <T>             annotation type
	...
	public static Object getFieldValue(Object target, String fieldName, boolean ignoreCase) {
		...
		return (field == null ? getPropertyValue(target, fieldName, (Object[]) null) : getFieldValue(target, field));
	}
	 ...
	 * If this field is static/public, value will be set via
	 * {@link Field#set(Object, Object)}, otherwise value will be set via
	 * {@link PropertyDescriptor#getWriteMethod()}
	 *
		...
		if (target != null && field != null) {
			...
			if (!Modifier.isFinal(field.getModifiers())
				...
				try {
					field.setAccessible(true);
					// try with number
					if (isInstanceOf(fClazz, Number.class) || isInstanceOf(fClazz, Boolean.class)) {
						Number numVal = null;
						...
						if (numVal != null && isInstanceOf(fClazz, BigDecimal.class)) {
							...
							field.set(target, numVal.intValue() == 1 ? Boolean.TRUE : Boolean.FALSE);
						} else if (numVal != null && isInstanceOf(fClazz, Boolean.class)) {
							field.set(target, numVal.intValue() == 1 ? Boolean.TRUE : Boolean.FALSE);
						} else
							applied = false;
						...
						} else if (isInstanceOf(value, Date.class) && isInstanceOf(fClazz, Timestamp.class)) {
							field.set(target, DateUtils.toTimestamp((Date) value));
						} else if (isInstanceOf(value, Date.class) && isInstanceOf(fClazz, Time.class)) {
	...
	/**
	 * Get all fields list that every field is also one of the specified annotations
	 * array
	 *
	 * @param <T>               annotation type
	 ...
	 *
	 * @return all fields list that every field is also one of the specified
	 *         annotations array or empty if failed
	 */
	 ...
	 * @param annotationClasses the annotation classes array to check
	 *
	 * @return a map of fields and annotations list of every field or empty if
	 *         failed
	 */
	public static <T extends Annotation> Map<Field, List<Annotation>> getMapFieldsBy(String beanClassName,
	 ...
	 *         failed
	 */
	public static <T extends Annotation> Map<Field, List<Annotation>> getMapFieldsBy(Class<?> beanClass,
			Class<T>... annotationClasses) {
	 ...
	 *         failed
	 */
	public static <T extends Annotation> Map<Field, List<Annotation>> getMapFieldsBy(Object target,
			Class<T>... annotationClasses) {
		return (target == null ? new LinkedHashMap<Field, List<Annotation>>()
	...
	public static <T extends Annotation> Map<Field, T> getMapFieldsBy(Object target, Class<T> annotationClass) {
		return (target == null ? new LinkedHashMap<Field, T>() : getMapFieldsBy(target.getClass(), annotationClass));
	}
...
//				LogUtils.logError(BeanUtils.class, e.getMessage());
	...
	/**
	 * Gets the annotation of the property in the specified entity class, property
	 * name and annotation class
	 *
	 * @param <T>             annotation type
	 ...
	 */
	public static <T extends Annotation> T getPropertyAnnotation(PropertyDescriptor property,
			Class<T> annotationClass) {
		T annotation = null;
	 ...
	 * @return the annotations list of the specified property
	 */
	public static <T extends Annotation> T getPropertyAnnotation(String targetClassName, String propName,
			Class<T> annotationClass) {
	 ...
	 * @return the annotations list of the specified property
	 */
	public static <T extends Annotation> T getPropertyAnnotation(Class<?> targetClass, String propName,
			Class<T> annotationClass) {
		return getPropertyAnnotation(getProperty(targetClass, propName), annotationClass);
	}

	/**
	 * Gets the annotation of the property in the specified entity instance,
	 * property name and annotation class
	 *
	 * @param <T>             annotation type
	 ...
	 * @return the annotations list of the specified property
	 */
	public static <T extends Annotation> T getPropertyAnnotation(Object target, String propName,
			Class<T> annotationClass) {
		return (target != null ? getPropertyAnnotation(target.getClass(), propName, annotationClass) : null);
	}

	/**
	 * Get the actual type arguments of the specified bean field (if field is
	 * generic type)
	 *
	...
	/**
	 * Get the first occurred actual type argument of the specified bean field (if
	 * field is generic type)
	 *
...
//					if (buff.length() > 0)
//						buff.append(", ");
//					buff.append(MessageFormat.format("{0} = {1}", property,
//							(fval == null ? "NULL" : String.valueOf(fval))));
//				}
//			} else {
...
//			LogUtils.logError(BeanUtils.class, e.getMessage());
		...
		return (!CollectionUtils.isEmpty(bytes) ? new ByteArrayInputStream(bytes) : null);
	}
...
//				LogUtils.logError(BeanUtils.class, e.getMessage());
	 ...
	 * @param properties property names array to export values. NULL for exporting
	 *                   all
	 *
	 * @return property values list such as matrix or empty/null if failed
	 */
//	public static <T> List<List<Object>> mapPropertyValuesList(Class<T> dataClass, List<T> dataLst,
//			String... properties) {
...
//	}
	 ...
	 * @return property values list such as matrix or empty/null if failed
	 */
//	public static <T> List<Map<String, Object>> mapPropertyValuesMap(Class<T> dataClass, List<T> dataLst,
//			String... properties) {
...
//	}
	...
	/**
	 * Map the specified data matrix to the object list by the properties array
	 * order
	 *
	 * @param <T>        data class type
	 * @param dataClass  data class
		...
		if (dataClass != null && !CollectionUtils.isEmpty(dataLst)) {
			...
			if (!CollectionUtils.isEmpty(propsLst)) {
				for (List<Object> data : dataLst) {
					...
					int maxProperties = NumberUtils.min(new int[] { propsLst.size(), data.size() });
